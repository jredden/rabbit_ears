...
public class TopicConsumer2Worker extends AbstractWorker implements Runnable {
	private static Logger log = LoggerFactory.getLogger(TopicConsumer2Worker.class);
	...
	public void setUp() {
		...
		try {
			queueName = super.getMessageOperations().queueNameAndExchangeTopic(channel, EXCHANGE3_NAME, getPatterns().get(2));
		} catch (MessageException me) {
			me.printStackTrace();
		}
	...
	public void simpleConsumerTest() {
		...
		while (true) {
			...
			log.info("#2 topic consumed message " + message);
			try {
				Thread.sleep(100);
