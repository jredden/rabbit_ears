...
public class RouteConsumer0Worker extends AbstractWorker implements Runnable{
	private static Logger log = LoggerFactory.getLogger(RouteConsumer0Worker.class);
	...
	public void setUp() {
		...
		try {
			queueName = super.getMessageOperations().queueNameAndExchangeRoute(channel, EXCHANGE2_NAME, getRoutes().get(0));
		} catch (MessageException me) {
			me.printStackTrace();
		}
	...
	public void simpleConsumerTest() {
		...
		while (true) {
			...
			log.info("route #0 consumed message " + message);
			try {
				Thread.sleep(100);
