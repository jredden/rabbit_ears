...
public class RouteConsumer2Worker extends AbstractWorker implements Runnable{
	private static Logger log = LoggerFactory.getLogger(RouteConsumer2Worker.class);
	...
	public void setUp() {
		...
		try {
			queueName = super.getMessageOperations().queueNameAndExchangeRoute(channel, EXCHANGE2_NAME, getRoutes().get(2));
		} catch (MessageException me) {
			me.printStackTrace();
		}
	...
	public void simpleConsumerTest() {
		...
		while (true) {
			...
			log.info("route #2 consumed message " + message);
			try {
				Thread.sleep(100);
